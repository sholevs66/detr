...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        # x = [batch, query=seq_len, channel]
        # 1) act on the query sequence for each channel - i.e., Wx(all_sequence_ch1), Wx(all_sequence_ch2), ...
        # 2) act on the on each individual query channels - i.e. Wx(query_1_all_channels), Wx(query_2_all_channels), ...
...
class MlpMixer(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.Sequential(*[
            block_layer(
                embed_dim, 400, mlp_ratio, mlp_layer=mlp_layer, norm_layer=norm_layer,
                act_layer=act_layer, drop=drop_rate, drop_path=drop_path_rate)
            for _ in range(num_blocks)])
        self.norm = norm_layer(embed_dim)
        #self.head = nn.Linear(embed_dim, self.num_classes) if num_classes > 0 else nn.Identity() # no need for transformer
    ...
    def forward_features(self, x):
        #import ipdb; ipdb.set_trace()
        #x = self.stem(x) # no need for this because resnet50 outputs [batch, 256, W/32, H/32] patches
        x = self.blocks(x)
        x = self.norm(x)
        #x = x.mean(dim=1) # no need for transformer
        return x

    def forward(self, x):
        x = self.forward_features(x)
        #x = self.head(x) # no need for transformer
        return x
